# Base hardware setup (from the original Open Duck Mini Runtime)
...
> preserved verbatim. It covers the one-time **base** setup: flashing the Pi, I2C,
> the serial latency timer, the speaker, the virtualenv + `pip install -e .`, and the
> first motor/IMU checks. Do this **once** on a fresh duck.
>
> For everything this **Supercharged v2.5** fork adds — the phone Web UI, the guided
> `first_time_setup.py` wizard, stability tuning, the antenna jitter fix, etc. — see
> the main [README](../README.md) and [What's New](WHATS_NEW.md).
